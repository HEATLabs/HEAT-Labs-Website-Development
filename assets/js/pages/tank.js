...
async function fetchAndDisplayRelatedGuides(tankSlug) {
    try {
        ...
        // Filter guides that are relevant to this tank
        const relevantGuides = allGuides.filter(guide => {
            ...
            // Check if 'allTanks' is in sections OR if the tank slug is in sections
            return sections.includes('allTanks') || sections.includes(tankSlug);
        });
...
// Function to update the Related Guides sidebar section for tanks
...
async function fetchTankData(tankId) {
    try {
        ...
        if (tank.slug) {
            await fetchAndDisplayRelatedGuides(tank.slug);
        }
...
function initializeAbilityModals() {
    ...
    document.querySelectorAll('.ability-card').forEach(card => {
        card.addEventListener('click', function() {
            ...
            fetch('https://raw.githubusercontent.com/HEATLabs/HEAT-Labs-Configs/refs/heads/main/tanks.json')
...
function populateTankStats(tankStats) {
    ...
    function updateStat(category, statName, value) {
        ...
        if (valueElement) {
            ...
            if (statName.includes('DEPRESSION') || statName.includes('ELEVATION')) {
                formattedValue = value.includes('+') || value.includes('-') ? value : value;
            }
